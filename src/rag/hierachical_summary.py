...
Bạn là bộ sinh câu hỏi pháp lý từ bản tóm tắt.
Sinh nhiều câu hỏi hữu ích để truy vấn GraphRAG.
Trả về JSON hợp lệ:
{
  "questions": ["...", "..."]
}
Yêu cầu:
- Câu hỏi ngắn gọn, rõ nghĩa, tiếng Việt.
- Không bịa thông tin ngoài nội dung tóm tắt.
- Không trùng lặp.
""".strip()
...
def _is_change_like_node(node: Dict[str, Any]) -> bool:
    ...
    keywords = [
        "sửa đổi", "bổ sung", "bãi bỏ", "thay thế", "có hiệu lực", "hết hiệu lực",
        "trách nhiệm thi hành", "điều khoản chuyển tiếp", "đính chính",
    ]
    return any(k in text for k in keywords)


def _build_summary_prompt(*, level: str, title: str, texts: List[str], metadata: Optional[Dict[str, Any]] = None, max_input_chars: int = 4500) -> str:
    metadata = metadata or {}
    packed_texts = _pack_texts(texts, max_chars=max_input_chars)
    joined_text = "\n\n".join(packed_texts)

    lines = [f"summary_level: {level}", f"title: {title}"]
...
def summarize_with_llm(*, level: str, title: str, texts: List[str], metadata: Optional[Dict[str, Any]] = None, max_input_chars: int = 4500) -> str:
    metadata = metadata or {}
...
def generate_questions_from_summary(summary_text: str, *, n_questions: int = 6) -> List[str]:
    summary_text = _norm_space(summary_text)
    if not summary_text:
        return []
    ...
    try:
        ...
        parsed = _safe_json_loads(getattr(resp, "content", ""), {"questions": []})
        questions = [
            _norm_space(str(q))
            for q in parsed.get("questions", [])
            if _norm_space(str(q))
        ]
    except Exception:
        questions = []

    if not questions:
        # fallback đơn giản từ summary
        sents = [s.strip() for s in re.split(r"(?<=[\.!?;])\s+", summary_text) if s.strip()]
        questions = [f"Nội dung pháp lý chính trong ý sau là gì: {s[:120]}?" for s in sents[:n_questions]]
...
def _make_summary_node(*, node_id: str, summary_level: str, text: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    md = dict(metadata or {})
...
def build_clause_summaries(graph: Dict[str, Any], *, max_input_chars: int = 3000) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for node in graph.get("nodes", []):
        if not _is_evidence_node(node) or _node_type_of(node) != "clause":
            continue
...
def build_article_summaries(graph: Dict[str, Any], *, max_input_chars: int = 4500, clause_summaries: Optional[List[Dict[str, Any]]] = None) -> List[Dict[str, Any]]:
    ...
    for article, nodes in sorted(by_article.items(), key=lambda x: _natural_sort_key(x[0])):
        ...
        legal_roles = sorted({str(_node_md(n).get("legal_role")) for n in selected_nodes if _node_md(n).get("legal_role")})
        ...
        summary = summarize_with_llm(
            ...
            title=article,
            texts=texts,
            metadata={"article": article, "legal_roles": legal_roles},
            max_input_chars=max_input_chars,
        )
...
def build_change_summaries(graph: Dict[str, Any], *, max_input_chars: int = 4500) -> List[Dict[str, Any]]:
    buckets: Dict[str, List[Dict[str, Any]]] = defaultdict(list)

    for node in graph.get("nodes", []):
        if not _is_evidence_node(node) or not _is_change_like_node(node):
            continue
    ...
    for role, nodes in sorted(buckets.items(), key=lambda x: _natural_sort_key(x[0])):
        ...
        summary = summarize_with_llm(
            ...
            title=role,
            texts=texts,
            metadata={"change_role": role, "articles": articles},
            max_input_chars=max_input_chars,
        )
...
def build_community_summaries(graph: Dict[str, Any], *, max_input_chars: int = 4500) -> List[Dict[str, Any]]:
    buckets: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    ...
    for role, nodes in sorted(buckets.items(), key=lambda x: _natural_sort_key(x[0])):
        ...
        summary = summarize_with_llm(
            ...
            title=role,
            texts=texts,
            metadata={"community_role": role, "articles": articles},
            max_input_chars=max_input_chars,
        )
...
def build_document_summary(*, document_title: Optional[str], article_summaries: List[Dict[str, Any]], change_summaries: List[Dict[str, Any]], community_summaries: Optional[List[Dict[str, Any]]] = None, max_input_chars: int = 5000) -> Optional[Dict[str, Any]]:
    ...
    return _make_summary_node(
        ...
        metadata={
            "document_title": title,
            "child_summary_ids": [s["node_id"] for s in article_summaries] + [s["node_id"] for s in change_summaries] + [s["node_id"] for s in community_summaries],
            "title": title,
        },
    )


def _refresh_graph_indexes(graph: Dict[str, Any]) -> Dict[str, Any]:
    nodes = list(graph.get("nodes", []))
    edges = list(graph.get("edges", []))

    node_index = {str(n.get("node_id")): n for n in nodes if n.get("node_id")}
    ...
    for e in edges:
        ...
        tgt = e.get("target_id")
        if src:
            adjacency[str(src)].append(e)
        if tgt:
            reverse[str(tgt)].append(e)

    graph["node_index"] = dict(node_index)
    graph["adjacency"] = dict(adjacency)
    graph["reverse_adjacency"] = dict(reverse)
    return graph


def attach_summary_edges(graph: Dict[str, Any], summary_nodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    edges = list(graph.get("edges", []))
    ...
    for s in summary_nodes:
        ...
        for target_id in md.get("source_node_ids", []) or []:
            if target_id:
                edges.append({"source_id": s["node_id"], "target_id": target_id, "relation_type": REL_SUMMARIZES})

        for child_id in md.get("child_summary_ids", []) or []:
            if child_id:
                edges.append({"source_id": s["node_id"], "target_id": child_id, "relation_type": REL_HAS_CHILD_SUMMARY})
    ...
    return _refresh_graph_indexes(graph)
...
def build_hierarchical_summaries(
    ...
    *,
    ...
    if include_clause:
        clause_summaries = build_clause_summaries(graph, max_input_chars=max_clause_input_chars)
    ...
    if include_change:
        change_summaries = build_change_summaries(graph, max_input_chars=max_change_input_chars)

    community_summaries: List[Dict[str, Any]] = []
    if include_community:
        community_summaries = build_community_summaries(graph, max_input_chars=max_community_input_chars)
    ...
    if generate_questions and document_summary:
        generated_questions = generate_questions_from_summary(document_summary.get("text", ""), n_questions=n_questions)

...
ENUMERATE_KEYWORDS = ["liệt kê", "gồm những", "gồm các", "các trường hợp", "từng khoản", "từng điểm", "bao gồm"]
SUMMARY_KEYWORDS = ["nói về gì", "thuộc nội dung nào", "tóm tắt", "khái quát", "bối cảnh", "ý nghĩa", "tinh thần chung"]
COMPARE_KEYWORDS = ["so sánh", "khác nhau", "đối chiếu", "phân biệt"]
PROCEDURE_KEYWORDS = ["thủ tục", "hồ sơ", "trình tự", "cách làm", "điều kiện", "quy trình"]
VALIDITY_KEYWORDS = ["còn hiệu lực", "hết hiệu lực", "hiệu lực", "hiện hành"]
GUIDANCE_KEYWORDS = ["hướng dẫn", "quy định chi tiết", "nghị định", "thông tư", "văn bản nào"]
...
- Không được trả lời nội dung pháp luật.
- Không tự thêm thông tin pháp lý ngoài câu hỏi gốc.
- Chỉ trả về JSON hợp lệ, không markdown.

Bạn phải cân nhắc đồng thời:
1. Câu hỏi nên ưu tiên trong cùng một văn bản hay nhiều văn bản.
2. Nên ưu tiên cùng cấp, cấp trên, cấp dưới hay phối hợp.
3. Có cần lôi thêm chunk cha, chunk con, chunk anh em, văn bản sửa đổi, văn bản hướng dẫn hay không.

Schema:
{
  "mode": "general | change | effective | sanction",
  "query_type": "locate | enumerate | summarize | compare | applicability | procedure | validity | cross_reference",
  "document_scope": "single_doc | multi_doc",
  "target_level": "document | article | clause | point",
  "level_strategy": "same_level | parent | children | sibling | mixed",
  "answer_granularity": "document | article | clause | point",
...
def detect_query_type(question: str) -> str:
    q = (question or "").lower()
    has_anchor = bool(ARTICLE_RE.search(q) or CLAUSE_RE.search(q) or POINT_RE.search(q))
    if any(k in q for k in COMPARE_KEYWORDS):
        return "compare"
    if any(k in q for k in ENUMERATE_KEYWORDS):
        return "enumerate"
    if any(k in q for k in PROCEDURE_KEYWORDS):
        return "procedure"
    if any(k in q for k in VALIDITY_KEYWORDS):
        return "validity"
    if any(k in q for k in CHANGE_KEYWORDS) or any(k in q for k in GUIDANCE_KEYWORDS):
...
def _derive_answer_granularity(*, target_level: str, level_strategy: str) -> str:
    target_level = str(target_level or "document")
    level_strategy = str(level_strategy or "mixed")
    if level_strategy == "children":
        return {
            "document": "article",
            "article": "clause",
            "clause": "point",
...
def _derive_needs(*, query_type: str, document_scope: str, level_strategy: str, mode: str) -> Dict[str, bool]:
    linked_docs = document_scope == "multi_doc"
    needs = {
        "self": query_type in {"locate", "enumerate", "summarize", "compare"} or document_scope == "single_doc",
        "parent": level_strategy in {"parent", "mixed"},
        "children": level_strategy in {"children", "mixed"},
        "siblings": level_strategy in {"same_level", "sibling", "mixed"} and query_type == "compare",
        "linked_docs": linked_docs,
        "amendments": mode in {"change", "effective"} or query_type in {"validity", "cross_reference"},
        "guidance_docs": linked_docs and query_type in {"procedure", "applicability", "cross_reference"},
    }
    return needs


def _finalize_query_profile(question: str, profile: Dict[str, Any]) -> Dict[str, Any]:
    q = (question or "").lower()
    explicit_refs = dict(profile.get("explicit_refs") or {})

    article_match = ARTICLE_RE.search(question or "")
    clause_match = CLAUSE_RE.search(question or "")
    point_match = POINT_RE.search(question or "")
    year_match = YEAR_RE.search(question or "")

    if article_match:
        explicit_refs["article"] = f"Điều {article_match.group(1)}"
    if clause_match:
        explicit_refs["clause"] = f"Khoản {clause_match.group(1)}"
    if point_match:
        explicit_refs["point"] = f"Điểm {point_match.group(1).lower()}"
    if year_match:
        try:
            explicit_refs["year"] = int(year_match.group(1))
        except Exception:
            pass

    profile["explicit_refs"] = explicit_refs

    if explicit_refs.get("point"):
        profile["target_level"] = "point"
    elif explicit_refs.get("clause"):
        profile["target_level"] = "clause"
    elif explicit_refs.get("article"):
        profile["target_level"] = "article"
    else:
        profile["target_level"] = str(profile.get("target_level") or "document")

    query_type = str(profile.get("query_type") or detect_query_type(question))
    mode = str(profile.get("mode") or detect_query_mode(question))

    if any(k in q for k in COMPARE_KEYWORDS):
        level_strategy = "sibling" if explicit_refs.get("article") or explicit_refs.get("clause") else "same_level"
    elif any(k in q for k in ENUMERATE_KEYWORDS):
        level_strategy = "children"
    elif any(k in q for k in SUMMARY_KEYWORDS):
        level_strategy = "parent"
    else:
        level_strategy = str(profile.get("level_strategy") or "same_level")

    if mode in {"change", "effective"} or any(k in q for k in PROCEDURE_KEYWORDS) or any(k in q for k in VALIDITY_KEYWORDS) or any(k in q for k in GUIDANCE_KEYWORDS):
        document_scope = "multi_doc"
    elif explicit_refs.get("article") or explicit_refs.get("clause") or explicit_refs.get("point"):
        document_scope = "single_doc"
    else:
        document_scope = str(profile.get("document_scope") or "multi_doc")

    if level_strategy not in {"same_level", "parent", "children", "sibling", "mixed"}:
        level_strategy = "mixed"
    if document_scope not in {"single_doc", "multi_doc"}:
        document_scope = "multi_doc"

    profile["mode"] = mode
    profile["query_type"] = query_type
    profile["document_scope"] = document_scope
    profile["level_strategy"] = level_strategy
    profile["answer_granularity"] = _derive_answer_granularity(
        target_level=str(profile.get("target_level") or "document"),
        level_strategy=level_strategy,
    )
    profile["needs"] = _derive_needs(
        query_type=query_type,
        document_scope=document_scope,
        level_strategy=level_strategy,
        mode=mode,
    )
    profile["needs_version_reasoning"] = bool(profile.get("needs_version_reasoning")) or profile["needs"]["amendments"]
    ...
    profile["reason"] = _norm_space(str(profile.get("reason") or ""))[:200]
...
def _fallback_query_profile(question: str, *, n_variants: int = 3) -> Dict[str, Any]:
    ...
    profile = {
        ...
        "document_scope": "single_doc" if (article_match or clause_match or point_match) else "multi_doc",
        "target_level": target_level,
        "level_strategy": "same_level" if (article_match or clause_match or point_match) else "mixed",
        "answer_granularity": target_level,
    ...
    return _finalize_query_profile(q, profile)
...
def analyze_query_with_llm(question: str, *, n_variants: int = 3) -> Dict[str, Any]:
    ...
    try:
        ...
        if query_type in {"locate", "enumerate", "summarize", "compare", "applicability", "procedure", "validity", "cross_reference"}:
            out["query_type"] = query_type

        document_scope = str(parsed.get("document_scope") or "").strip().lower()
        if document_scope in {"single_doc", "multi_doc"}:
            out["document_scope"] = document_scope
        ...
        level_strategy = str(parsed.get("level_strategy") or "").strip().lower()
        if level_strategy in {"same_level", "parent", "children", "sibling", "mixed"}:
            out["level_strategy"] = level_strategy

        answer_granularity = str(parsed.get("answer_granularity") or "").strip().lower()
        if answer_granularity in {"document", "article", "clause", "point"}:
            out["answer_granularity"] = answer_granularity
        ...
        if isinstance(llm_refs, dict):
            for key in ["document_name", "article", "clause", "point", "year"]:
                value = llm_refs.get(key)
                if value not in (None, ""):
                    explicit_refs[key] = value
        out["explicit_refs"] = explicit_refs

        llm_needs = parsed.get("needs") or {}
        if isinstance(llm_needs, dict):
            base_needs = dict(out.get("needs") or {})
            for key in ["self", "parent", "children", "siblings", "linked_docs", "amendments", "guidance_docs"]:
                if key in llm_needs:
                    base_needs[key] = bool(llm_needs.get(key))
            out["needs"] = base_needs

        out["confidence"] = parsed.get("confidence", out.get("confidence", 0.0))
        out["reason"] = parsed.get("reason", out.get("reason", ""))
        ...
        return _finalize_query_profile(q, out)
...
def compute_scope_score(query_profile: Dict[str, Any], metadata: Dict[str, Any]) -> float:
    ...
    level_strategy = str(query_profile.get("level_strategy") or "mixed")

    target_rank = LEVEL_ORDER.get(target_level, 0)
    node_rank = LEVEL_ORDER.get(node_level, 0)

    score = 0.0
    if level_strategy in {"same_level", "sibling"}:
        if node_level == target_level:
            score += 0.44
        elif abs(node_rank - target_rank) == 1:
            score += 0.14
        else:
            score += 0.04
    elif level_strategy == "parent":
        if node_rank < target_rank:
            score += 0.44
        elif node_level == target_level:
            score += 0.20
        else:
            score += 0.05
    elif level_strategy == "children":
        if node_rank > target_rank:
            score += 0.44
        elif node_level == target_level:
            score += 0.20
...
def build_final_passage_pool(
    ...
    *,
    ...
    document_scope = str(query_profile.get("document_scope") or "multi_doc")
    level_strategy = str(query_profile.get("level_strategy") or "mixed")
    needs = dict(query_profile.get("needs") or {})

    if document_scope == "single_doc":
        top_docs = 1
        per_doc_base = max(per_doc_base, 4 if level_strategy == "children" else 3)
        rescue_global = min(rescue_global, 2)
    elif needs.get("linked_docs") or needs.get("amendments") or needs.get("guidance_docs"):
        top_docs = max(top_docs, 4)
        if level_strategy == "parent":
            per_doc_base = max(2, per_doc_base - 1)
